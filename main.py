...
def get_generators() -> dict:
    ...
    prefix = "generate_"
    return {
        strip_prefix(n, prefix): getattr(generators, n)
...
def get_verifiers() -> dict:
    ...
    prefix = "verify_"
    return {
        strip_prefix(n, prefix): getattr(verifiers, n)
        for n in dir(verifiers)
        if n.startswith(prefix)
    }
...
def demo_dataset(folder: str = "re_arc", n: int = 8, s: int = 0, e: int = 400) -> None:
    """
    ...
    for i, fn in enumerate(sorted(os.listdir(f"{folder}/tasks"))):
        if s <= i < e:
            ...
            original_task = [
                ...
                for example in original_task["train"] + original_task["test"]
            ]
            generated_task = [
                format_example(example) for example in generated_task[: 10 * n]
            ]
            difficulties = metadata[key]["pso_difficulties"][: 9 * n]
            generated_task = [
                ex
                for ex, diff in sorted(
...
def evaluate_verifiers_on_original_tasks() -> None:
    ...
    for key in verifiers.keys():
        with open(f"../data/{key}.json", "r") as fp:
            task = json.load(fp)
